...
export default {
  activate() {
    ...
    var pkgSubscription = atom.packages.onDidActivatePackage(pkg => {
  ...
  startUsingLevels() {
    if (!this.usingLevels) {
      try {
        if (!this.language) {
          const configFilePath = CSON.resolve(path.join(this.pkgLanguageDirPath, 'config'));
          ...
          const dummyGrammarPath = CSON.resolve(path.join(this.pkgLanguageDirPath, 'grammars', 'dummy'));
        ...
        atom.grammars.addGrammar(this.dummyGrammar);
  ...
  stopUsingLevels() {
    if (this.usingLevels) {
      atom.grammars.removeGrammar(this.dummyGrammar);
  ...
  setUpConfigManagement() {
    ...
    this.configSubscription = atom.config.onDidChange(configKeyPath, ({newValue}) => {
      return this.setExecutionCommandPatterns(newValue);
    });
  }
};